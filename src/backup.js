...
const fail = () => { throw new Error('Backup inválido ou incompatível. Nenhum registro foi alterado.'); };
...
const network = (value, solana = false) => {
  if (!Object.hasOwn(CHAINS, value) || (solana && CHAINS[value].family !== 'solana')) fail();
  return value;
};
const signature = value => {
  field(value, 88);
  try { if (bs58.decode(value).length !== 64) fail(); } catch { fail(); }
  return value;
};

// File values are hints only: receipt amounts and cached balances are never restored.
export function mergeBackup(current, raw) {
  if (typeof raw !== 'string' || new TextEncoder().encode(raw).length > MAX_BACKUP_BYTES) fail();
  let data;
  try { data = JSON.parse(raw); } catch { fail(); }
  if (!data || data.version !== 1) fail();
  const next = structuredClone(current);
  let walletsAdded = 0, invoicesAdded = 0, signaturesAdded = 0;
  ...
  for (const i of list(data.invoices, 500)) {
    ...
    const lot = field(i.lot, 200), contract = field(i.contract ?? '', 500, true);
    const createdAt = field(i.createdAt, 40);
    if (!/^\d{4}-\d\d-\d\dT/.test(createdAt) || !Number.isFinite(Date.parse(createdAt))) fail();
    const candidates = [...list(i.receipts ?? [], 500).map(r => signature(r?.signature)), ...list(i.pendingSignatures ?? [], 500).map(signature)];
    let target = next.invoices.find(x => x.chain === chain && x.reference === reference);
    if (target) {
      if (target.recipient !== recipient || units(target.amount) !== units(amount) || target.lot !== lot || target.contract !== contract || Date.parse(target.createdAt) !== Date.parse(createdAt))
        throw new Error('Uma cobrança existente usa esta referência com dados diferentes. Importação cancelada.');
    } else {
      target = { id: crypto.randomUUID(), chain, recipient, reference, amount, lot, contract, createdAt, receipts: [], pendingSignatures: [] };
      next.invoices.push(target);
      invoicesAdded++;
    }
    target.pendingSignatures ??= [];
    for (const s of new Set(candidates)) {
      if (!target.pendingSignatures.includes(s) && !next.invoices.some(x => x.chain === chain && x.receipts.some(r => r.signature === s))) {
