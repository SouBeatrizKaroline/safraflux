...
export const CHAINS = {
  ...
  'solana-devnet': {name: 'Solana Devnet', family: 'solana', rpc: 'https://api.devnet.solana.com', symbol: 'SOL de teste', decimals: 9, mint: '4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU', explorer: 'https://explorer.solana.com', test: true},
  base: {name: 'Base', family: 'evm', rpc: 'https://mainnet.base.org', chainId: '0x2105', symbol: 'ETH', decimals: 18, mint: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', explorer: 'https://basescan.org'},
  ethereum: {name: 'Ethereum', family: 'evm', rpc: 'https://ethereum-rpc.publicnode.com', chainId: '0x1', symbol: 'ETH', decimals: 18, mint: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', explorer: 'https://etherscan.io'},
...
export async function rpc(endpoint, method, params = []) {
  const r = await fetch(endpoint, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({jsonrpc: '2.0', id: 1, method, params}), signal: AbortSignal.timeout(18000)});
  if (!r.ok) throw new Error(`Consulta indisponível (HTTP ${r.status}). Tente outro RPC em Configurações.`);
  const data = await r.json();
  if (data.error) throw new Error('O RPC recusou a consulta. Verifique a rede e o limite do provedor.');
...
export async function balances(wallet, endpoints = {}) {
  ...
  if (chain.family === 'solana') {
    ...
    if (!Number.isSafeInteger(native.value)) throw new Error('Saldo nativo excede a precisão suportada pelo RPC JSON.');
    const usdc = tokens.value.reduce((sum, t) => sum + BigInt(t.account.data.parsed.info.tokenAmount.amount), 0n);
    return {native: String(native.value), usdc: String(usdc), at: new Date().toISOString()};
  }
  const chainId = await rpc(endpoint, 'eth_chainId');
  if (chainId.toLowerCase() !== chain.chainId) throw new Error('O RPC respondeu por uma rede diferente.');
  const [native, usdc] = await Promise.all([
    rpc(endpoint, 'eth_getBalance', [address, 'latest']),
    rpc(endpoint, 'eth_call', [{to: chain.mint, data: '0x70a08231' + address.slice(2).padStart(64, '0')}, 'latest'])
  ]);
...
export function paymentURI(invoice) {
  ...
  if (!chain || chain.family !== 'solana') throw new Error('Cobrança disponível apenas na Solana.');
  const q = new URLSearchParams({'amount': invoice.amount, 'spl-token': chain.mint, reference: invoice.reference, label: 'SafraFlux', message: `Pagamento do lote ${invoice.lot}`});
...
export function solanaWallets() {
  return getWallets().get().filter(w => w.features['standard:connect'] && w.chains.some(c => c.startsWith('solana:')));
}

export async function connectSolana(wallet, chain) {
  const {accounts} = await wallet.features['standard:connect'].connect();
  const network = chain === 'solana-devnet' ? 'solana:devnet' : 'solana:mainnet';
  const account = accounts.find(a => a.chains.includes(network));
  if (!account) throw new Error('Esta carteira não disponibilizou uma conta para a rede selecionada.');
...
if (typeof window !== 'undefined') {
  window.addEventListener('eip6963:announceProvider', event => {
    ...
    if (info?.uuid && provider?.request) evmProviders.set(info.uuid, {info, provider});
  });
...
export async function connectEVM(provider, chain) {
  ...
  if (chainId.toLowerCase() !== CHAINS[chain].chainId) throw new Error(`Selecione a rede ${CHAINS[chain].name} na carteira e tente novamente.`);
  const accounts = await provider.request({method: 'eth_requestAccounts'});
  if (!accounts?.length) throw new Error('Nenhuma conta autorizada.');
  return accounts[0];
}

export function explorerTx(chain, signature) { return `${CHAINS[chain].explorer}/tx/${encodeURIComponent(signature)}${CHAINS[chain].test ? '?cluster=devnet' : ''}`; }
