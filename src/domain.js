...
export function units(value, decimals = 6) {
  ...
  if (!/^\d+(\.\d+)?$/.test(raw))
    throw new Error("Use um valor positivo, sem separador de milhar.");
...
export function allocate(total, weights) {
  ...
  if (amount < 0n || !values.length || values.some((w) => w < 0n))
    throw new Error("Rateio inválido.");
  const sum = values.reduce((a, b) => a + b, 0n);
  if (sum === 0n) {
    if (amount !== 0n)
      throw new Error("Informe pesos para distribuir o valor.");
    return values.map(() => 0n);
  }
  const result = values.map((w) => (amount * w) / sum);
  const order = values
    .map((w, i) => ({ i, remainder: (amount * w) % sum }))
    .sort((a, b) =>
...
export function calculateSplit(total, producers, premiumPercent = "0") {
  ...
  if (!producers.length || producers.length > 100)
    throw new Error("Informe de 1 a 100 produtores.");
  const kg = producers.map((p) => units(p.kg, 3));
  if (kg.some((k) => k === 0n))
    throw new Error("Cada produtor deve ter quantidade maior que zero.");
  const scores = producers.map((p) => units(p.points, 2));
  if (scores.some((n) => n > 10000n))
    throw new Error("Pontos de qualidade: de 0 a 100.");
  const pool = (amount * percent) / 10000n;
  const base = allocate(amount - pool, kg);
  const premium = allocate(
    pool,
    kg.map((k, i) => k * scores[i]),
  );
  return producers.map((p, i) => ({
    ...p,
    base: decimal(base[i]),
    premium: decimal(premium[i]),
    total: decimal(base[i] + premium[i]),
  }));
}

export function invoiceStatus(invoice) {
  const total = (invoice.receipts || []).reduce(
  ...
  return {
    ...
    outstanding: decimal(total < expected ? expected - total : 0n),
    excess: decimal(total > expected ? total - expected : 0n),
    status:
      total === 0n
        ? "Pendente"
        : total < expected
...
export function appendReceipt(invoices, invoiceId, receipt) {
  ...
  if (
    invoices.some(
      (i) =>
        i.chain === invoice.chain &&
        i.receipts.some((r) => r.signature === receipt.signature),
    )
  )
...
export function formatAmount(value) {
  ...
  return (
    BigInt(integer).toLocaleString("pt-BR") + (fraction ? "," + fraction : "")
  );
}
...
export function receiptFromTransaction(
  ...
  if (!tx || !tx.meta || tx.meta.err)
    throw new Error("Transação ausente ou sem execução bem-sucedida.");
  if (tx.transaction.signatures[0] !== signature)
    throw new Error("A assinatura retornada não corresponde à consulta.");
  ...
  if (referenceIndex < 0)
    throw new Error("A transação não contém a referência desta cobrança.");
  const header = message.header;
  const readOnly =
    referenceIndex < staticKeys.length
      ? referenceIndex >= header.numRequiredSignatures &&
        referenceIndex >= staticKeys.length - header.numReadonlyUnsignedAccounts
      : referenceIndex >= staticKeys.length + loaded.writable.length;
  if (!readOnly)
    throw new Error(
      "A referência precisa ser somente leitura e não assinante.",
    );
  if (
    !tx.blockTime ||
    tx.blockTime * 1000 < Date.parse(invoice.createdAt) - 300000
  )
    throw new Error(
      "A transação é anterior à cobrança ou não possui data verificável.",
    );
  const destinationIndex = keys.indexOf(destinationATA);
  if (destinationIndex < 0)
    throw new Error("A conta USDC de destino não está nesta transação.");
  let transferred = 0n;
  for (const instruction of message.instructions) {
    ...
    if (checked && (keys[accounts[1]] !== mint || data[9] !== 6))
      throw new Error("Token ou precisão incorretos na transferência.");
    transferred += new DataView(
      data.buffer,
      data.byteOffset,
      data.byteLength,
    ).getBigUint64(1, true);
  ...
  if (transferred <= 0n)
    throw new Error(
      "Não há transferência SPL direta para o destino com a referência desta cobrança.",
    );
  const balance = (list) =>
    (list || [])
      .filter(
        (b) =>
          b.accountIndex === destinationIndex &&
          b.mint === mint &&
          b.owner === invoice.recipient,
      )
      .reduce((sum, b) => {
        if (b.uiTokenAmount.decimals !== 6)
          throw new Error("Precisão do token diferente do USDC esperado.");
        return sum + BigInt(b.uiTokenAmount.amount);
      }, 0n);
  const amount =
    balance(tx.meta.postTokenBalances) - balance(tx.meta.preTokenBalances);
  if (amount <= 0n)
    throw new Error(
      "Não houve entrada líquida de USDC no destinatário desta cobrança.",
    );
  if (amount !== transferred)
    throw new Error(
      "A transação contém movimentações adicionais no destino. Use uma transferência direta por cobrança.",
    );
  return {
    signature,
