...
export function localApi() {
  return {
    name: "safraflux-local-api",
    configureServer(server) {
      mkdirSync(".local", { recursive: true });
      const db = sqliteAdapter(".local/safraflux.sqlite");
      db.exec(
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (host !== "127.0.0.1:4173" && host !== "localhost:4173") {
            res.statusCode = 403;
            res.end();
            return;
          }
          ...
          for (const [k, v] of Object.entries(req.headers))
            if (v && !k.startsWith("oai-")) headers.set(k, String(v));
          if (process.env.SAFRAFLUX_LOCAL_TEST === "1")
            headers.set("oai-authenticated-user-id", "local-test-user");
          ...
          res.statusCode = 503;
          res.end('{"error":"Servidor local indisponível"}');
        }
      });
