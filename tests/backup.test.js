...
const invoice = () => ({
  ...
  receipts: [{ signature: sig, amount: "10000000" }],
});
const empty = () => ({
  wallets: [],
  invoices: [],
  endpoints: { base: "https://example.org" },
});
const backup = (invoices = [invoice()], wallets = []) =>
  JSON.stringify({
    version: 1,
    invoices,
    wallets,
    endpoints: { base: "https://malicious.example" },
  });
...
test("conflito de referência rejeita todo o arquivo sem mutar o estado", () => {
  ...
  assert.throws(() =>
    mergeBackup(original, backup([{ ...invoice(), amount: "11" }])),
  );
...
test("rejeita versões, redes, valores, assinaturas e arquivos inválidos", () => {
  for (const raw of [
    ...
    "{}",
    "{",
    JSON.stringify({ version: 2, wallets: [], invoices: [] }),
    " ".repeat(MAX_BACKUP_BYTES + 1),
  ])
    assert.throws(() => mergeBackup(empty(), raw));
  for (const patch of [
    { chain: "__proto__" },
    { chain: "base" },
    { amount: "-1" },
    ...
    assert.throws(() =>
      mergeBackup(empty(), backup([{ ...invoice(), ...patch }])),
    );
});
...
test("limite de carteiras se aplica ao resultado da mesclagem", () => {
  ...
  assert.throws(() =>
    mergeBackup(
      { ...empty(), wallets },
      backup([], [{ name: "Nova", chain: "solana-devnet", address }]),
    ),
  );
});
